...
export default function Analytics({ state }: { state:LearnerState }) {
  ...
  // All-tracks overview: readiness and streak for every certification, active one first.
  const tracks=certifications.map(c=>{
    const ca=state.attempts.filter(a=>a.certId===c.id);
    return { id:c.id, name:c.name, shortName:c.shortName, active:c.id===cert,
  ...
  return <><PageHead/>
    {tracks.length>1 && <div className="panel"><div className="panel-title"><div><span>ALL TRACKS</span><h3>Progress across certifications</h3></div></div><div className="domain-list">{tracks.map(t=><div className="domain-row" key={t.id}><span className="domain-dot" style={{background:t.active?"#36d6b5":"var(--muted)"}}/><div><b>{t.name}{t.active?" · active":""}</b><small>{t.exams?`${t.exams} exam${t.exams>1?"s":""} · ${t.streak} day streak`:"No attempts yet"}</small></div><div className="thin-progress"><i style={{width:`${t.readiness}%`,background:t.active?"#36d6b5":"var(--muted)"}}/></div><strong>{t.readiness}%</strong></div>)}</div></div>}
