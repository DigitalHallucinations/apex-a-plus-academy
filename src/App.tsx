...
import {
  ...
  Download, Layers3, Menu, Moon, NotebookPen, Play, Plus, RotateCcw, Search, Settings, ShieldCheck,
...
const initialState: LearnerState = {
  ...
  answered: {}, attempts: [], bookmarks: [], notes: [], cardRatings: {}, dailyActivity: {}, theme: "dark"
};
...
function recordStudy(state: LearnerState, questionsDone = 0, cardsDone = 0): LearnerState {
  const today = dayKey();
  const current = state.dailyActivity[today] || { questions: 0, cards: 0 };
  let streak = state.streak || 0;
  if (state.lastStudyDate !== today) streak = state.lastStudyDate && daysBetween(state.lastStudyDate, today) === 1 ? streak + 1 : 1;
  return { ...state, streak, lastStudyDate: today, dailyActivity: { ...state.dailyActivity, [today]: { questions: current.questions + questionsDone, cards: current.cards + cardsDone } } };
}
...
export default function App() {
  ...
  return <div className={`app ${sidebar ? "" : "collapsed"}`}>
    <aside className="sidebar">
      ...
      <nav>{nav.map(item => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => { setView(item.id); if(window.innerWidth<=760)setSidebar(false); }} title={item.label}><item.icon/><span>{item.label}</span></button>)}</nav>
    ...
    <main>
      <header>
        ...
        <button className="search" onClick={() => setSearchOpen(true)}><Search/><span>Search objectives, commands, ports...</span><kbd>Ctrl K</kbd></button>
        <div className="notice-wrap"><button className="icon-btn" aria-label="Study notifications" onClick={() => setNoticesOpen(x=>!x)}><Bell/><i/></button>{noticesOpen&&<StudyNotices state={state} close={()=>setNoticesOpen(false)} openView={(next)=>{setView(next);setNoticesOpen(false)}}/>}</div>
      ...
      <section className="content">
        {view === "dashboard" && <Dashboard state={state} setView={setView} startDomain={(id) => { setPracticeDomain(id); setView("practice"); }} />}
        {view === "learn" && <Learn state={state} setState={setState} startDomain={(id) => { setPracticeDomain(id); setView("practice"); }} />}
        {view === "practice" && <Practice state={state} setState={setState} initialDomain={practiceDomain} clearDomain={() => setPracticeDomain(null)} />}
        ...
        {view === "settings" && <Preferences state={state} update={update} replaceState={setState} />}
      </section>
    </main>
    {searchOpen && <GlobalSearch query={searchQuery} setQuery={setSearchQuery} close={() => setSearchOpen(false)} openView={(next) => { setView(next); setSearchOpen(false); }} startDomain={(id) => { setPracticeDomain(id); setView("practice"); setSearchOpen(false); }} />}
  </div>;
}

function Dashboard({ state, setView, startDomain }: { state: LearnerState; setView: (v: View) => void; startDomain: (id:string) => void }) {
  ...
  const todayQuestions = state.dailyActivity[dayKey()]?.questions || 0;
...
function Learn({ state, setState, startDomain }: { state:LearnerState; setState:React.Dispatch<React.SetStateAction<LearnerState>>; startDomain:(id:string)=>void }) {
...
function Practice({ state, setState, initialDomain, clearDomain }: { state:LearnerState; setState:React.Dispatch<React.SetStateAction<LearnerState>>; initialDomain:string|null; clearDomain:()=>void }) {
  ...
  const start = () => { const pool=initialDomain?questions.filter(q=>q.domain===initialDomain):exam==="Mixed"?questions:questions.filter(q=>q.exam===exam); setSession(shuffle(pool).slice(0,Math.min(count,pool.length)));setIndex(0);setAnswers({});setChecked({});setElapsed(0);setMode("active"); };
  const finish = () => {
    ...
    setState(s=>{const answered={...s.answered};session.forEach(q=>{const old=answered[q.id]||{correct:0,attempts:0};answered[q.id]={correct:old.correct+(answers[q.id]===q.answer?1:0),attempts:old.attempts+1};});return recordStudy({...s,answered,attempts:[...s.attempts,attempt]},session.length,0);});clearDomain();setMode("results");
  };
  if(mode==="setup") {
    const focus=domains.find(d=>d.id===initialDomain);
    const available=focus?questions.filter(q=>q.domain===focus.id).length:exam==="Mixed"?questions.length:questions.filter(q=>q.exam===exam).length;
    const countOptions=Array.from(new Set([Math.min(5,available),10,15,20])).filter(x=>x>0);
    return <><PageHead eyebrow="ADAPTIVE PRACTICE" title="Practice lab" subtitle={focus?`Focused drill: ${focus.name}`:"Choose a target, enter focus mode, and learn from every explanation."}/><div className="setup-grid"><div className="panel setup-main"><h3>{focus?`${focus.name} drill`:"Build your session"}</h3>{focus?<button className="focus-banner" onClick={clearDomain}><Target/><div><b>{focus.name}</b><small>{focus.exam} · {available} available questions</small></div><X/></button>:<><label>Exam track</label><div className="option-grid">{(["220-1201","220-1202","Mixed"] as const).map(x=><button key={x} className={exam===x?"selected":""} onClick={()=>setExam(x)}><span>{x==="Mixed"?<Brain/>:x==="220-1201"?<Activity/>:<ShieldCheck/>}</span><b>{x}</b><small>{x==="Mixed"?"Both cores":"Focused objective mix"}</small></button>)}</div></>}<label>Question count</label><div className="count-picker">{countOptions.map(x=><button key={x} className={Math.min(count,available)===x?"selected":""} disabled={x>available} onClick={()=>setCount(x)}>{x}</button>)}</div><button className="primary wide launch" onClick={start}><Play/> Launch session</button></div><div className="panel setup-side"><span className="pill purple"><Sparkles/> SESSION PREVIEW</span><h2>{Math.min(count,available)} questions</h2><div className="preview-row"><Clock3/><div><b>Estimated time</b><small>{Math.min(count,available)*1.5} minutes</small></div></div><div className="preview-row"><Target/><div><b>Coverage</b><small>{focus?focus.name:exam === "Mixed" ? "All Core 1 & Core 2 domains" : `Weighted ${exam} mix`}</small></div></div><div className="preview-row"><CircleHelp/><div><b>Learning mode</b><small>Explanations available after answering</small></div></div></div></div></>;
  }
...
function Flashcards({ state, setState }: { state:LearnerState; setState:React.Dispatch<React.SetStateAction<LearnerState>> }) {
  ...
  const [reviewAll,setReviewAll]=useState(false); const deck=reviewAll?flashcards:due; const [index,setIndex]=useState(0); const [flipped,setFlipped]=useState(false); const card=deck[index%Math.max(1,deck.length)];
  const rate=(ease:number)=>{if(!reviewAll){const prev=state.cardRatings[card.id]||{ease:2.5,interval:0,due:""};const interval=ease===1?1:Math.max(1,Math.round((prev.interval||1)*(ease===2?1.5:ease===3?2.5:4)));const dueDate=new Date(Date.now()+interval*86400000).toISOString();setState(s=>recordStudy({...s,cardRatings:{...s.cardRatings,[card.id]:{ease,interval,due:dueDate}}},0,1));}setIndex(i=>i+1);setFlipped(false);};
  if(reviewAll&&index>=deck.length) return <><PageHead eyebrow="REVIEW COMPLETE" title="Recall deck" subtitle="You reviewed every card without changing the spaced-repetition schedule."/><div className="panel deck-complete"><div className="result-ring pass"><Check/><span>Full deck reviewed</span></div><h2>Excellent recall workout.</h2><p>Your scheduled due dates are unchanged. Return to the normal queue whenever you are ready.</p><button className="ghost" onClick={()=>{setReviewAll(false);setIndex(0)}}><ChevronLeft/> Return to due queue</button></div></>;
  if(!deck.length) return <><PageHead eyebrow="SPACED REPETITION" title="Recall deck" subtitle="Actively retrieve the answer, then rate your recall honestly."/><div className="panel deck-complete"><div className="result-ring pass"><Check/><span>Queue complete</span></div><h2>Nothing else is due today.</h2><p>You cleared the scheduled recall queue. Come back tomorrow or review the full deck without changing its schedule.</p><button className="ghost" onClick={()=>{setReviewAll(true);setIndex(0)}}><RotateCcw/> Review all cards</button></div></>;
...
function Notes({ state, setState }: { state:LearnerState; setState:React.Dispatch<React.SetStateAction<LearnerState>> }) {
  const [active,setActive]=useState(""); const [title,setTitle]=useState(""); const [body,setBody]=useState("");
  useEffect(()=>{if(!active&&state.notes[0]){setActive(state.notes[0].id);setTitle(state.notes[0].title);setBody(state.notes[0].body)}},[active,state.notes]);
...
function Preferences({ state, update, replaceState }: { state:LearnerState; update:(n:Partial<LearnerState>)=>void; replaceState:React.Dispatch<React.SetStateAction<LearnerState>> }) {
  const [notice,setNotice]=useState("");
  const backup=()=>{const blob=new Blob([JSON.stringify(state,null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`apex-a-plus-backup-${dayKey()}.json`;a.click();URL.revokeObjectURL(url);setNotice("Backup downloaded.");};
  const reset=()=>{if(window.confirm("Reset all progress, notes, bookmarks, and settings? This cannot be undone unless you exported a backup.")){replaceState({...initialState,theme:state.theme});setNotice("Local learner data reset.");}};
  return <><PageHead eyebrow="MAKE IT YOURS" title="Preferences" subtitle="Tune your study target, daily rhythm, and workspace."/><div className="settings-grid"><div className="panel settings-card"><div className="setting-icon"><GraduationCap/></div><div><h3>Learner profile</h3><p>This name appears throughout your workspace.</p><label>Display name<input value={state.name} onChange={e=>update({name:e.target.value})}/></label></div></div><div className="panel settings-card"><div className="setting-icon"><CalendarDays/></div><div><h3>Exam target</h3><p>Set a date to add a countdown to your dashboard.</p><label>Target date<input type="date" value={state.targetDate} onChange={e=>update({targetDate:e.target.value})}/></label></div></div><div className="panel settings-card"><div className="setting-icon"><Target/></div><div><h3>Daily mission</h3><p>Choose a realistic question goal you can sustain.</p><label>Questions per day<input type="number" min="5" max="100" value={state.dailyGoal} onChange={e=>update({dailyGoal:Math.max(5,Math.min(100,Number(e.target.value)))})}/></label></div></div><div className="panel settings-card"><div className="setting-icon"><Moon/></div><div><h3>Appearance</h3><p>Switch the complete interface theme.</p><div className="theme-toggle"><button className={state.theme==="dark"?"active":""} onClick={()=>update({theme:"dark"})}><Moon/> Dark</button><button className={state.theme==="light"?"active":""} onClick={()=>update({theme:"light"})}><Sun/> Light</button></div></div></div><div className="panel settings-card"><div className="setting-icon"><Download/></div><div><h3>Local data</h3><p>Download a portable JSON backup or start over.</p><div className="data-actions"><button className="ghost" onClick={backup}><Download/> Export backup</button><button className="danger" onClick={reset}><Trash2/> Reset data</button></div>{notice&&<small className="setting-notice">{notice}</small>}</div></div><div className="panel about-card"><div className="brand-mark"><Zap/></div><div><h3>Apex A+ Academy</h3><p>Version 1.1.0 · Offline-first desktop edition</p><small>Your progress is stored locally on this computer. This independent educational app is not affiliated with or endorsed by CompTIA.</small></div></div></div></>;
}

function StudyNotices({state,close,openView}:{state:LearnerState;close:()=>void;openView:(view:View)=>void}) {
  const today=state.dailyActivity[dayKey()]?.questions||0;
  const due=flashcards.filter(f=>!state.cardRatings[f.id]||state.cardRatings[f.id].due<=new Date().toISOString()).length;
  return <div className="notice-popover panel"><div className="notice-head"><div><span>STUDY STATUS</span><h3>Today at a glance</h3></div><button onClick={close}><X/></button></div><button onClick={()=>openView("practice")}><Target/><div><b>{Math.max(0,state.dailyGoal-today)} questions left</b><small>{today} of {state.dailyGoal} completed today</small></div><ChevronRight/></button><button onClick={()=>openView("flashcards")}><Layers3/><div><b>{due} flashcards due</b><small>{due?"Keep your recall queue moving":"Your scheduled queue is clear"}</small></div><ChevronRight/></button><div className="notice-streak"><Flame/><span>{state.streak} day study streak</span></div></div>;
}

function GlobalSearch({query,setQuery,close,openView,startDomain}:{query:string;setQuery:(q:string)=>void;close:()=>void;openView:(v:View)=>void;startDomain:(id:string)=>void}) {
  const q=query.trim().toLowerCase();
  const results=q?[...domains.filter(d=>`${d.name} ${d.description} ${d.topics.join(" ")} ${d.exam}`.toLowerCase().includes(q)).map(d=>({id:d.id,type:"Domain",title:d.name,sub:`${d.exam} · ${d.description}`,action:()=>startDomain(d.id)})),...questions.filter(x=>`${x.prompt} ${x.objective} ${x.explanation} ${x.options.join(" ")}`.toLowerCase().includes(q)).slice(0,8).map(x=>({id:x.id,type:"Question",title:x.objective,sub:x.prompt,action:()=>startDomain(x.domain)})),...flashcards.filter(x=>`${x.front} ${x.back}`.toLowerCase().includes(q)).slice(0,6).map(x=>({id:x.id,type:"Flashcard",title:x.front,sub:x.back,action:()=>openView("flashcards")}))]:[];
  return <div className="search-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="search-dialog panel"><div className="search-input"><Search/><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Try DHCP, malware, NVMe, backups..."/><button onClick={close}><X/></button></div><div className="search-results">{!q?<div className="search-hint"><span>Search all domains, objectives, questions, answers, and flashcards.</span><small>Press Esc to close</small></div>:results.length?results.map(r=><button key={`${r.type}-${r.id}`} onClick={r.action}><span>{r.type}</span><div><b>{r.title}</b><small>{r.sub}</small></div><ChevronRight/></button>):<Empty message={`No study content found for “${query}”.`}/>}</div></div></div>;
}
